...
export interface ModelController {
  ...
  /** Cycle to the next advertised model in place (click-to-switch). */
  cycleModel(): void
  /** Cycle the selected model's reasoning effort in place (click-to-switch). */
  cycleEffort(): void
...
export function createModelController(deps: ModelControllerDeps): ModelController {
  ...
  /** Run a catalog read, or report the failure once; disposed calls are no-ops. */
  const withChoices = async (use: (choices: readonly ModelChoice[]) => void): Promise<void> => {
    try {
  ...
  const cycleModel = (): void => {
    void withChoices((choices) => {
      if (choices.length === 0) {
        deps.appendNotice('No models are advertised by registered providers.', 'warning')
        return
      }
      ...
      /* v8 ignore next -- a non-empty list always yields an entry. */
  ...
  const cycleEffort = (): void => {
    void withChoices((choices) => {
      const current = target.current
      const choice = current === undefined
        ? undefined
        : choices.find(candidate => candidate.provider === current.provider && candidate.model === current.model)
      if (choice === undefined || current === undefined) {
        deps.appendNotice('Select a model before cycling reasoning effort.', 'warning')
        return
      }
      const efforts = choice.reasoning?.efforts ?? []
      const ids: (ReasoningEffortId | undefined)[] = [undefined, ...efforts.map(effort => effort.id)]
      const next = ids[(ids.indexOf(current.reasoningEffort) + 1) % ids.length]
      if (next === current.reasoningEffort) {
        deps.appendNotice('This model has no other reasoning efforts.', 'warning')
        return
      }
      selectModel(choice, { effort: next })
    })
  }
