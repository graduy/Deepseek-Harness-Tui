...
/** SGR extended mouse sequence: `ESC[<button;column;rowM` (press) / `m` (release). */
const MOUSE_SGR = /^\x1b\[<(\d+);(\d+);(\d+)([Mm])$/u

/** A full-width horizontal rule used to frame the pinned input box. */
class RuleComponent implements Component {
  constructor(private readonly paint: (text: string) => string) {}
  invalidate(): void {}
...
/**
 * A spacer that grows to pin the input box to the terminal's bottom edge: it
 * fills however many rows remain after the transcript region (above) and the
 * fixed input area (below), so the editor never floats under a short transcript.
 */
class BottomSpacer implements Component {
  constructor(
    private readonly above: Component,
    private readonly below: Component,
    private readonly rows: () => number,
  ) {}
  invalidate(): void {}
  render(width: number): string[] {
    const fill = Math.max(0, this.rows() - this.above.render(width).length - this.below.render(width).length)
    return new Array<string>(fill).fill('')
  }
}

/**
 * A one-line status bar: model and reasoning-effort flush-left (each a
 * clickable segment), the file name flush-right (the bottom-right corner).
 * Column ranges of the clickable segments are published each render so the
 * mouse handler can hit-test clicks against them.
 */
class StatusBarComponent implements Component {
  /** 1-based inclusive column ranges for the clickable left segments. */
  readonly segments: { model?: readonly [number, number]; effort?: readonly [number, number] } = {}
  ...
  render(width: number): string[] {
    ...
    const separator = ' · 思考强度 '
    const modelStart = 1
    this.segments.model = [modelStart, modelStart + visibleWidth(model) - 1]
    const effortStart = modelStart + visibleWidth(model) + visibleWidth(separator)
    this.segments.effort = [effortStart, effortStart + visibleWidth(effort) - 1]
    const left = model + separator + effort
    if (file === '') return [this.paint(left)]
    const gap = Math.max(1, width - visibleWidth(left) - visibleWidth(file))
    return [this.paint(left + ' '.repeat(gap) + file)]
  }
}
...
export function createTuiChat(
  ...
  /** 24-bit color is available: enable the clay accent and brand art. */
  const truecolor = resolved.theme.color && resolved.theme.truecolor
  const palette = createPalette(resolved.theme.color, 'dark', truecolor)
  ...
  let reasoningVisibility = resolved.reasoningVisibility
  ...
  // The companion extension's active-file payload (workspace stays internal).
  let ideContext: IdeContext | undefined
  let ideTimer: ReturnType<typeof setInterval> | undefined
  const refreshIdeContext = (): void => {
    const next = readIdeContext(ideContextPath())
    const changed = next !== undefined && (
      ideContext === undefined
      || next.updatedAt !== ideContext.updatedAt
      || next.activeFile !== ideContext.activeFile
      || next.workspace !== ideContext.workspace
    )
    if (next !== undefined && !changed) return
    ideContext = next
    const file = formatIdeFile(ideContext)
    ideFileText = file === '' ? '' : displayText(file)
    requestRender()
  }
  ...
  const updatePromptValues = (): void => {
    ...
    // The pinned status bar under the input box: model + reasoning effort on
    // the left (both clickable), file name right-aligned.
    modelText = target.current === undefined ? 'model unset' : target.current.model
    effortText = target.current?.reasoningEffort ?? 'default'
  }
  ...
  // The transcript region: whale banner, conversation, todo/compaction/context
  // lines, and inline question modals. Kept in one container so the bottom
  // spacer can measure it and pin the input box to the terminal's bottom edge.
  const aboveContainer = new Container()
  aboveContainer.addChild(header)
  aboveContainer.addChild(chat)
  aboveContainer.addChild(new Spacer(1))
  todoContainer.addChild(todo)
  aboveContainer.addChild(todoContainer)
  aboveContainer.addChild(compactionStatusLine)
  aboveContainer.addChild(promptContext)
  aboveContainer.addChild(questionContainer)

  // The fixed input area, framed by two horizontal rules with the model status
  // line underneath — Claude Code-style, pinned to the bottom by BottomSpacer.
  const topRule = new RuleComponent(text => palette.dim(text))
  const bottomRule = new RuleComponent(text => palette.dim(text))
  const belowContainer = new Container()
  belowContainer.addChild(topRule)
  belowContainer.addChild(editor)
  belowContainer.addChild(bottomRule)
  belowContainer.addChild(statusBar)

  ui.addChild(aboveContainer)
  ui.addChild(new BottomSpacer(aboveContainer, belowContainer, () => runtime.terminal.rows))
  ui.addChild(belowContainer)
  ui.setFocus(editor)
    ...
    switch (event.type) {
      case 'user/message': {
        if (isSteering) {
          ...
          if (steeringText) {
            chat.addChild(new Spacer(1))
            chat.addChild(new UserMessageComponent(steeringText, palette))
          }
          break
        }
        ...
        if (text) {
          insertAboveCurrentStep(new Spacer(1), new UserMessageComponent(text, palette))
  ...
  const shutdown = (exitProcess: boolean): Promise<void> => {
    shuttingDown ??= (async () => {
      ...
      runtime.terminal.write('\x1b[?1000l\x1b[?1006l')
      ui.stop()
      if (exitProcess) {
  ...
  const setReasoning = (next: ReasoningVisibility): void => {
    reasoningVisibility = next
    ...
    if (activeStreaming !== undefined) {
      ...
      streaming.setReasoningVisibility(reasoningVisibility)
      registerAssistantStep(activeStreaming)
      chat.addChild(activeStreaming)
      chat.addChild(activeStreaming.timing)
    }
    appendNotice(`Reasoning ${reasoningVisibility === 'hidden' ? 'hidden' : reasoningVisibility === 'expanded' ? 'expanded' : 'collapsed'}.`)
  }

  const toggleReasoning = (): void => {
    const index = REASONING_PHASES.indexOf(reasoningVisibility)
    setReasoning(REASONING_PHASES[(index + 1) % REASONING_PHASES.length] as ReasoningVisibility)
  }
    ...
    const session = overlayManager.open({
      create: () => new DetailsDialog(
        ...
        (selection: DetailsSelection) => {
          if (selection.reasoningVisibility !== reasoningVisibility) setReasoning(selection.reasoningVisibility)
  ...
  const runDetails = (rawInput: string): CommandResult => {
    ...
    let reasoning: ReasoningVisibility | undefined
    for (let token = tokens.shift(); token !== undefined; token = tokens.shift()) {
      if (token === 'collapsed' || token === 'expanded' || token === 'hidden') {
        ...
        if (value === 'on' || value === 'off' || value === 'collapsed' || value === 'expanded' || value === 'hidden') {
          tokens.shift()
          reasoning = value === 'on' ? 'expanded' : value === 'off' ? 'hidden' : value
        } else {
          const index = REASONING_PHASES.indexOf(reasoningVisibility)
          reasoning = REASONING_PHASES[(index + 1) % REASONING_PHASES.length] as ReasoningVisibility
        }
      } else {
        return { kind: 'error', text: `Unknown /details argument "${token}". Usage: /details [collapsed|expanded|hidden] [reasoning [collapsed|expanded|hidden|on|off]]` }
      }
    }
  ...
  const showHelp = (): void => {
    ...
    chat.addChild(new Text([
      ...
      'Esc cancel turn • Ctrl+O cycle cards (collapse/expand/hide) • Ctrl+R or click cycle reasoning • Ctrl+L redraw',
  ...
  const showStatus = async (signal: AbortSignal): Promise<void> => {
    ...
    const groups: readonly (readonly StatusCardRow[])[] = [
      [
        ...
        ['Model', `${model} ${palette.dim(`(effort ${effort}; reasoning ${reasoningVisibility})`)}`],
      ],
      [
        ['Agent', [
          agent.status,
  ...
  const removeInputListener = ui.addInputListener((data) => {
    ...
    // SGR extended mouse (`ESC[<b;x;yM/m`): a left-button press on the status
    // bar switches model / reasoning effort in place; anywhere else it toggles
    // reasoning between the collapsed disclosure and the expanded text.
    const mouse = MOUSE_SGR.exec(data)
    if (mouse !== null) {
      if (mouse[1] === '0' && mouse[4] === 'M') {
        const column = Number(mouse[2])
        const row = Number(mouse[3])
        const model = statusBar.segments.model
        const effort = statusBar.segments.effort
        if (row === runtime.terminal.rows && model !== undefined && column >= model[0] && column <= model[1]) {
          modelController.cycleModel()
        } else if (row === runtime.terminal.rows && effort !== undefined && column >= effort[0] && column <= effort[1]) {
          modelController.cycleEffort()
        } else {
          setReasoning(reasoningVisibility === 'collapsed' ? 'expanded' : 'collapsed')
        }
      }
      return { consume: true }
    }
  ...
  const detachListeners = (): void => {
    skillAbort.abort()
    fileSearch.dispose()
    if (ideTimer !== undefined) clearInterval(ideTimer)
    removeInputListener()
  ...
  // Enable SGR extended mouse reporting so a left click can toggle reasoning.
  runtime.terminal.write('\x1b[?1000h\x1b[?1006h')
  // Start watching the companion extension's IDE-context payload.
  refreshIdeContext()
  ideTimer = setInterval(refreshIdeContext, 1500)
