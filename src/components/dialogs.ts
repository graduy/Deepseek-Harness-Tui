...
import { REASONING_PHASES } from '../config.ts'
import {
...
export class DetailsDialog implements Component {
  ...
  private reasoningLabel(): string {
    return this.reasoningVisibility
  }
  ...
  private cycle(): void {
    ...
    if (selected.value === 'tools') {
      ...
      const index = REASONING_PHASES.indexOf(this.reasoningVisibility)
      this.reasoningVisibility = REASONING_PHASES[(index + 1) % REASONING_PHASES.length] as ReasoningVisibility
      this.reasoningItem.description = this.reasoningLabel()
    }
    this.apply({ visibility: this.visibility, reasoningVisibility: this.reasoningVisibility })
  }
