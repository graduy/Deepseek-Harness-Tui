 ...
 * @param truecolor - Use Claude Code's warm clay accent on 24-bit terminals.
 * @returns The SGR spec for every color and attribute role.
 */
export function paletteSpec(scheme: TerminalColorScheme, truecolor = false): {
  ...
  return {
    colors: {
      ...
      // The one emphasis tone: role headers, prompt, borders, links, headings.
      // Truecolor terminals get Claude Code's warm clay #D97757; the 16-color
      // fallback keeps bright magenta so the emphasis still reads on any scheme.
      accent: truecolor
        ? { open: '38;2;217;119;87', close: '39', purpose: 'The one emphasis color: role headers, prompt, borders' }
        : { open: '95', close: '39', purpose: 'The one emphasis color: role headers, prompt, borders' },
 ...
 * @param truecolor - Use the clay accent on 24-bit terminals.
 * @returns The role palette for the given scheme.
 */
export function createPalette(enabled: boolean, scheme: TerminalColorScheme = 'dark', truecolor = false): Palette {
