...
/**
 * Reasoning-block visibility across the transcript. `collapsed` renders a
 * one-line disclosure header per message (Claude Code-style), `expanded`
 * renders the full dimmed text, and `hidden` drops reasoning entirely.
 */
export type ReasoningVisibility = 'collapsed' | 'expanded' | 'hidden'

/** The three reasoning states in Ctrl+R cycle order. */
export const REASONING_PHASES: readonly ReasoningVisibility[] = ['collapsed', 'expanded', 'hidden']

/** Interaction and presentation settings for the pi-tui terminal mode. */
export interface TuiConfig {
  /** Render model reasoning blocks: collapsed disclosure, full text, or none. */
...
const reasoningVisibilitySchema = z.union([
  z.const('collapsed'),
  z.const('expanded'),
  z.const('hidden'),
]).default('collapsed')
...
const tuiConfigSchemaFields = {
  reasoningVisibility: reasoningVisibilitySchema,
...
export const Config: z<Config> = z.object({
  ...
  reasoningVisibility: tuiConfigSchemaFields.reasoningVisibility,
...
export function resolveTuiConfig(config: TuiConfig | undefined): ResolvedTuiConfig {
  return {
    reasoningVisibility: config?.reasoningVisibility ?? 'collapsed',
